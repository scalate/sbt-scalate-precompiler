...
class Precompiler extends PrecompilerInterface {
    ...
    compileTargets.collect {
      ...
      case Updated(uri, templateFile, scalaFile) =>
        ...
        try {
          ...
          case e: Exception => {
            throw new Exception(
              "Template compilation failed at file %s, Exception: %s" format (
  ...
  private object CompileTarget {
     ...
     */
    private def isModified(templateFile: File, scalaFile: File): Boolean =
      templateFile.lastModified > scalaFile.lastModified
